...
<form method='post' action='${request.route_url("genaf.marker-action")}'>
${selection_bar('marker', ('Add Marker', request.route_url("genaf.marker-edit", id=0)))}
...
% for m in markers:
  <tr><td><input type="checkbox" name="marker-ids" value="${m.id}" /></td>
      <td><a href="${request.route_url('genaf.marker-view', id=m.id)}">${m.code}</a></td>
      <td>${m.locus}</td>
      <td>${m.species}</td>
...
<form class="form-horizontal form-condensed"
        method='POST'
        action='${request.route_url("genaf.marker-save", id=marker.id)}' >
  <fieldset>
    ${input_hidden('genaf/marker.id', '', value=marker.id)}
    ${input_text('genaf/marker.code', 'Marker Code', value=marker.code)}
    ${input_text('genaf/marker.locus', 'Locus', value=marker.locus)}
    ${input_text('genaf/marker.species', 'Species', value=marker.species)}
